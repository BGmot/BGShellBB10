	 ...
	 d(new K3ProcessPrivate)
{
  ...
  //QObject::connect(ps, SIGNAL(started()), this, SLOT(on_cmdShell_start())); //+++
  //QObject::connect(ps, SIGNAL(finished(int)), this, SLOT(on_cmdShell_finish(int))); //+++
  //QObject::connect(ps, SIGNAL(readyRead()), this, SLOT(on_cmdShell_data_available())); //+++
...
/*+++
  ...
  //QStringList *args = new QStringList("111");
  //+++ps->start(program, QStringList() << "!!!");
...
/*+++
  ...
  if (pid_ == 0) {
	   ...
	   if (geteuid() != getuid())
        ...
        // forking failed
	    perror("K3Process::start - fork() failed:");
        // commAbort();
        pid_ = 0;
        free(arglist);
        return false;
...
void K3Process::slotSendData(int)
{
  if (input_sent == input_total) {
	...
	//+++int result = ps->write(input_data+input_sent, input_total-input_sent);
    if (result >= 0)
    {
       input_sent += result;
    }
...
/*//+++V
void K3Process::on_cmdShell_start(){
	qDebug() << "process started";
}
void K3Process::on_cmdShell_finish(int){
	qDebug() << "process exited";
}
void K3Process::on_cmdShell_data_available(){

    int len = ps->bytesAvailable();
    QByteArray bufferQ = ps->readAll();
    bufferQ.append('0');
    //len = ::read(fdno, buffer, 1024);

    if (len > 0) {
       //buffer[len] = 0; // Just in case.
       emit receivedStdout(this, bufferQ.data(), bufferQ.length());
    }
    return;
}
//+++A*/
